...
class qemuLauncher:
    ...
    def build_docker_image(self, tag, path):
    ...
    def build_qemu(self, tag=QEMU_BUILDER_IMAGE, build_cache=QEMU_BUILD_CACHE_VOLUME, args=[]):
        """build qemu using docker qemu builder image"""
        # connect to docker daemon
        client = docker.from_env()
        # build qemu builder image
        docker_path = Path(self.REPO_ROOT, "docker/qemu-builder")
        self.build_docker_image(tag, docker_path)
        ...
        # run docker build
        mounts = {str(self.REPO_ROOT): {"bind": "/qemu/src", "mode": "rw"},
    ...
    def run_qemu(self, tag=QEMU_RUNNER_IMAGE, instances=1, args=[]):
        """run qemu"""
        # build qemu runner image
        docker_path = Path(self.REPO_ROOT, "docker/qemu")
        self.build_docker_image(tag, docker_path)
        # run qemu
        try:
            for i in range(instances):
                p = mp.Process(target=self.run_qemu_instance, args=(i,))
                p.start()        
                self.processes.append(p)  # Keep list of processes (maybe just pids?) to clean up later?
        except KeyboardInterrupt:
            pass
    
    def run_qemu_instance(self, i):
        docker_path = Path(self.REPO_ROOT, "docker/qemu")
    ...
    def stop_qemu(self, instances=1, args=[]):
        """stop all docker services and cleanup resources"""
        docker_path = Path(self.REPO_ROOT, "docker")
        for i in range(instances):
...
if __name__ == "__main__":
    ...
    parser_build.add_argument("--tag", default=ql.QEMU_BUILDER_IMAGE, help="qemu docker builder image")
    parser_build.add_argument("--build-cache", default=ql.QEMU_BUILD_CACHE_VOLUME, help="qemu build cache volume")
    parser_build.set_defaults(fn=ql.build_qemu)
    # run subcommand
    parser_run = sp.add_parser("run", help="run qemu")
    parser_run.add_argument("--tag", default=ql.QEMU_RUNNER_IMAGE, help="qemu docker runner image")
    parser_run.add_argument("-n", default=1, dest="instances", type=int, help="number of instances")
    parser_run.set_defaults(fn=ql.run_qemu)
    # cleanup subcommand
    parser_run = sp.add_parser("stop", help="stop and cleanup all docker containers, volumes, etc")
    parser_run.add_argument("-n", default=1, dest="instances", type=int, help="number of instances")
    parser_run.set_defaults(fn=ql.stop_qemu)
